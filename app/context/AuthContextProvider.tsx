...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      if (
        ...
        parseUser.userType &&
        (parseUser.userType === "admin" || parseUser.userType === "faculty")
      ) {
        setUser(parseUser);
  ...
  const login = (userData: User, sessionType: SessionType = "permanent") => {
    ...
    if (sessionType === "temporary")
      sessionStorage.setItem(authStorageSessionKey, JSON.stringify(userData));
    else if (sessionType === "permanent")
      localStorage.setItem(authStorageSessionKey, JSON.stringify(userData));
    else {
