              ...
              <div className="col-span-2 flex flex-col justify-between items-end">
                <div className="flex flex-col items-end">
                  ...
                  <div className="text-md">
                    {capitalizeString(daysShort[new Date().getDay()])},{" "}
                ...
                <div className="text-right">
                  <div className="font-mono text-[12px]">
                    {weatherData.latitude &&
                      formatDegreeToDMS(weatherData.latitude, "latitude")}{" "}
                    {weatherData.longitude &&
                      formatDegreeToDMS(weatherData.longitude, "longitude")}
                  </div>
                  <div className="text-md font-semibold">Imphal</div>
                </div>
              </div>
            </div>
          </div>
          ...
          >
            {detailView && (
              ...
              >
                {detailView &&
                  detailData &&
                  Array.from(detailData).map(([k, v], index) => {
                    if (k.trim().toLowerCase() === "time") {
                      return (
                        <React.Fragment key={`time-${index}`}>
                          <div className="hover:bg-white/10 py-0 px-2 md:px-0.5 font-sans font-thin text-sm text-slate-50 border-b border-b-slate-50 mx-0.5 inline-flex flex-nowrap items-center justify-between ">
                            ...
                            <div className="font font-semibold">
                              {moment(new Date(v)).format("DD/MM/YYYY")}
                            </div>
                          </div>
                          <div
                            key={"Time"}
                      ...
                      return (
                        ...
                        >
                          <div>
                            {capitalizeString(formatSnakeCaseString(k))}:
                          </div>
