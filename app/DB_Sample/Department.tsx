

export const Department_data = [
  {
    DeptCode: "CSE",
    DeptName: "Computer Science and Engineering",
    Desc: "The Department presently offers the following programmes: \n\t  1. B.E. in Computer Science & Engineering \n\t  2. M. Tech in Computer Science & Engineering \n\t  3. Ph. D. in Computer Science & Engineering",
    Images: [
      {
        caption: "Lab_1",
        link: "Images/Department/CSE/img_1.jpg",
      },
      {
        caption: "Lab_1",
        link: "Images/Department/CSE/img_2.jpg",
      },
    ],
    MoreInfo: [
      {
        Facilities:
          "The Department has several state of the art Computer Laboratories with a host of servers, workstation and a large number of i3 and i5 Desktop Computers connected to the Campus-wide LAN with access to the internet through NKN (National Knowledge Network). The system runs on wide variety of operating systems including Linux Red hat, Windows 7 Professional, Windows 8, High profile Anti-Virus, MySQL, Oracle, Visual Studio, Adobe Premier, Maya (Latest). The laboratories are equipped with up-to-date office automation software, file servers in addition to the various state-of-the-art compilers and programming environment i.e. Hardware Lab., Software Lab., Networking Lab., and Graphic Lab. well equipped with latest Core i3, i5 PC’s etc. The network laboratory is equipped with wireless networking system, LAN trainers and Wi-Fi connection. There is a hardware Lab. equipped with various training kits, experimental setup, analyser equipment etc. \n\n1. PCs: Core i3 and i5 Desktop Computers and Data Server are available in various laboratories. \n2. INTERNET :\n          High speed Internet access with separate Cable connectivity for academic, All Departments, Hostels, Library and browsing centre.\n          Wi-Fi Campus",
      },
    ],
  },
  // EE
  {
    DeptCode: "EE",
    DeptName: "Department of Electrical Engineering",
    Desc: "The Department is offering B.E. in Electrical Engineering (intake 30) from the academic session 2019-20.",
    Images: [
      {
        caption: "",
        link: "",
      },
      {
        caption: "",
        link: "",
      },
    ],
    MoreInfo: [
      {
        Facilities: "",
      },
    ],
  },
  // BSH
  {
    DeptCode: "BSH",
    DeptName: "Basic Sciences & Humanities",
    Desc: "Faculty Member of Basic Sciences & Humanities\n    The Basic Science & Humanities Department of Manipur Institute of Technology has the following faculty members as arrayed below:-",
    Images: [
      {
        caption: "",
        link: "",
      },
      {
        caption: "",
        link: "",
      },
    ],
    MoreInfo: [
      {
        Facilities:
          "",
      },
    ],
  },
  // ECE
  {
    DeptCode: "ECE",
    DeptName: "Electronics & Communication Engineering",
    Desc: "The Department presently offers the following programmes:\n1. B.E. in Electronics & Communication Engineering\n2. M. Tech in the following specialisations :\n   a) Microelectronics and VSLI\n   b) Communication and Signal Processing\n3. Ph. D. in Electronics & Communication Engineering",
    Images: [
      {
        caption: "",
        link: "",
      },
      {
        caption: "",
        link: "",
      },
    ],
    MoreInfo: [
      {
        Facilities: "",
      },
    ],
  },
  // CE
  {
    DeptCode: "CE",
    DeptName: "Civil Engineering",
    Desc: "Civil Engineering department presently offers the following programmes: \n1. B.E. in Civil Engineering \n2. M. Tech. in the following specialisations\n    a. Structural Engineering\n    b. Geotechnical Engineering\n    c. Water Resources Engineering\n    d. Transportation Engineering\n    e. Environmental Engineering\n3. Ph. D. in Civil Engineering \n\n<strong>Vision:</strong>To be a leader in Civil Engineering education by imparting quality technical knowledge to achieve excellence in academic research, industry and entrepreneurship.\n\n <strong>Mission:</strong>\n1. To empower students and faculty with broad knowledge in Civil Engineering and applications.\n2. To produce Civil engineers, capable of handling technical and social challenges.\n3. To produce entrepreneurs capable of solving present problems of the society.\n4. To provide technological services which are sustainable and environment friendly.\n\n<strong>Programme Educational Objectives (PEOs):</strong>\n 1. Graduates will be actively engaged in a professional career as a civil engineer or a related field, an entrepreneur or pursuing higher study.\n2. Graduates will understand professional practice and demonstrate a commitment to act as a responsible, effective and ethical citizen undertaking lifelong learning.\n3. Graduates guided by principles of sustainable development will understand how their activities as a civil engineer or an entrepreneur or in a related field affect society and environment.\n 4.Graduates will develop their communication skills when working as team members or leaders, so that they can actively participate in their communities and their profession.",
    Images: [
      {
        caption: "Lab_1",
        link: "Images/Department/CE/lab_1.jpg",
      },
      {
        caption: "Lab_1",
        link: "Images/Department/CE/lab_2.jpg",
      },
    ],
    MoreInfo: [
      {
        Facilities: "Other Major Equipments are:-\n   1. Microprocessor based UV-visible Spectrophotometer\n   2. Tri-Axial testing Apparatus\n   3. Portable DO Meter\n   4. Digital Control BOD Incubator\n   5. Trimble Total Station\n   6. Digital Flame Photometer\n   7. Electrical Analogy Apparatus\n   8. Hele-Shaw Apparatus\n   9. Water Hammer Surge Apparatus\n  10. Marshal Apparatus",
      },
    ],
  },
  // ME
  {
    DeptCode: "ME",
    DeptName: "Mechanical Engineering",
    Desc: "The Department is offering B.E. in Electrical Engineering (intake 30) from the academic session 2019-20.",
    Images: [
      {
        caption: "",
        link: "",
      },
      {
        caption: "",
        link: "",
      },
    ],
    MoreInfo: [
      {
        Facilities: "",
      },
    ],
  },
];
